...
@section('content')
    <div class="mb-5">
        <x-breadcrumb :items="[
            ...
            'Registros de atendimento AEE' => route('specialized-educational-support.students.aee-records.index', $student),
            'Visualizar' => route('specialized-educational-support.students.aee-records.show', [$student, $evaluation]),
            'Editar' => null
        ]" />
    </div>
    ...
    <div class="d-flex justify-content-between mb-3">
        <div>
            ...
            <p class="text-muted">
                {{ $student->person->name }} •
                Registro #{{ $evaluation->aeeRecord->id }} •
                Agendamento #{{ $evaluation->aeeRecord->attendance_session_id }}
            </p>
        </div>

        <x-buttons.link-button
            href="{{ route('specialized-educational-support.students.aee-records.show', [$student, $evaluation]) }}"
            variant="secondary">
            <i class="fas fa-times"></i> Cancelar
        </x-buttons.link-button>
    </div>

    <x-forms.form-card action="{{ route('specialized-educational-support.students.aee-records.update', [$student, $evaluation]) }}" method="POST">
        @method('PUT')
        ...
        <div class="col-12 d-flex justify-content-between gap-3 border-t pt-4 px-4 pb-4 mt-4">
            ...
            <div class="d-flex gap-2">
                <x-buttons.link-button
                    href="{{ route('specialized-educational-support.aee-records.show', $aeeRecord) }}"
                    variant="info">
