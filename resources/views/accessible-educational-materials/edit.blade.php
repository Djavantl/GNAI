...
<div class="max-w-4xl mx-auto bg-white p-6 rounded shadow">
    <div class="flex justify-between items-center mb-6 border-b pb-4">
        ...
        <span class="text-sm text-gray-500 font-mono bg-gray-100 px-2 py-1 rounded">ID: #{{ $accessibleEducationalMaterial->id }}</span>
    </div>
    ...
    <form action="{{ route('accessible-educational-materials.update', $accessibleEducationalMaterial) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="type" value="{{ old('type', $accessibleEducationalMaterial->type) }}" class="w-full border p-2 rounded">
                </div>
                <div>
                    <label class="block font-medium">Formato</label>
                    <input type="text" name="format" value="{{ old('format', $accessibleEducationalMaterial->format) }}" class="w-full border p-2 rounded">
                </div>
            </div>
            ...
            @if($accessibleEducationalMaterial->images->count() > 0)
                <div class="mt-2">
                    <label class="block font-bold mb-2 text-gray-700">Imagens Atuais ({{ $accessibleEducationalMaterial->images->count() }})</label>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="language" value="{{ old('language', $accessibleEducationalMaterial->language) }}" class="w-full border p-2 rounded">
                </div>
                <div>
                    <label class="block font-medium">ISBN</label>
                    <input type="text" name="isbn" value="{{ old('isbn', $accessibleEducationalMaterial->isbn) }}" class="w-full border p-2 rounded">
                </div>
                <div>
                    <label class="block font-medium">Nº de Páginas</label>
                    <input type="number" name="pages" value="{{ old('pages', $accessibleEducationalMaterial->pages) }}" class="w-full border p-2 rounded">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="publisher" value="{{ old('publisher', $accessibleEducationalMaterial->publisher) }}" class="w-full border p-2 rounded">
                </div>
                <div>
                    <label class="block font-medium">Edição</label>
                    <input type="text" name="edition" value="{{ old('edition', $accessibleEducationalMaterial->edition) }}" class="w-full border p-2 rounded">
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                    @foreach(App\Models\AccessibilityFeature::where('is_active', true)->get() as $feature)
                        <div class="flex items-center gap-2">
                            ...
                            <input type="checkbox" name="accessibility_features[]" value="{{ $feature->id }}" id="acc_{{ $feature->id }}"
                                {{ in_array($feature->id, old('accessibility_features', $accessibleEducationalMaterial->accessibilities->pluck('id')->toArray())) ? 'checked' : '' }}>
                            <label for="acc_{{ $feature->id }}" class="text-sm cursor-pointer">{{ $feature->name }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="asset_code" value="{{ old('asset_code', $accessibleEducationalMaterial->asset_code) }}" class="w-full border p-2 rounded">
                </div>
                <div>
                    <label class="block font-medium">Custo (R$)</label>
                    <input type="number" step="0.01" name="cost" value="{{ old('cost', $accessibleEducationalMaterial->cost) }}" class="w-full border p-2 rounded">
                </div>
                <div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('accessible-educational-materials.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded transition flex items-center">
                    Cancelar
                </a>
            </div>
        </div>
    ...
    @foreach($accessibleEducationalMaterial->images as $image)
        <form id="delete-image-{{ $image->id }}"
              action="{{ route('accessible-educational-materials.images.destroy', $image->id) }}"
              method="POST"
              class="hidden">
            @csrf
            @method('DELETE')
