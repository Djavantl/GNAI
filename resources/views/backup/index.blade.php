                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-4 px-4 text-right align-middle">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('backup.backups.edit', $backup->id) }}" class="text-blue-600 hover:bg-blue-50 px-3 py-1 rounded transition border border-blue-100 text-sm font-semibold">
                                    <i class="fas fa-edit"></i>
                                </a>

                                <a href="{{ route('backup.backups.download', $backup->id) }}" class="text-green-600 hover:bg-green-50 px-3 py-1 rounded transition border border-green-100 text-sm font-semibold">
                                    <i class="fas fa-download"></i>
                                </a>

                                <form action="{{ route('backup.backups.destroy', $backup->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
