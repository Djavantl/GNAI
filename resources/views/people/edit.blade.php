        ...
        <form action="{{ route('people.update', $person) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $person->name) }}" class="w-full border p-2 rounded">
                    @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block font-semibold">Documento (CPF)</label>
                    <input type="text" name="document" value="{{ old('document', $person->document) }}" class="w-full border p-2 rounded">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="birth_date" value="{{ old('birth_date', $person->birth_date?->format('Y-m-d')) }}" class="w-full border p-2 rounded">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $person->email) }}" class="w-full border p-2 rounded">
                </div>

                <div>
                    <label class="block font-semibold">Telefone</label>
                    <input type="text" name="phone" value="{{ old('phone', $person->phone) }}" class="w-full border p-2 rounded">
                </div>

                <div class="flex gap-4 mt-4">
                    <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Atualizar</button>
                    <a href="{{ route('people.index') }}" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">Cancelar</a>
                </div>
            </div>
        </form>
    </div>
