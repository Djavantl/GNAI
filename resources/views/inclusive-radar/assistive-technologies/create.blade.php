    ...
    <form action="{{ route('inclusive-radar.assistive-technologies.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 gap-4">
                    ...
                    @foreach(\App\Models\SpecializedEducationalSupport\Deficiency::where('is_active', true)->get() as $def)
                        <div class="flex items-center gap-2">
                            ...
                            <label for="def_{{ $def->id }}"
                                   class="text-sm cursor-pointer hover:text-blue-600 transition">{{ $def->name }}</label>
                        </div>
                    @endforeach
                </div>
                ...
                <div>
                    ...
                    <select name="assistive_technology_status_id" class="w-full border p-2 rounded">
                        ...
                        @foreach(\App\Models\InclusiveRadar\AssistiveTechnologyStatus::where('is_active', true)->get() as $status)
                            <option
                                value="{{ $status->id }}" {{ old('assistive_technology_status_id') == $status->id ? 'selected' : '' }}>
                                {{ $status->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="w-full border p-2 rounded"
            ...
            <div class="flex flex-col gap-2 p-3 bg-gray-100 rounded border border-gray-300">
                <div class="flex items-center gap-2">
                    ...
                    <label for="requires_training" class="cursor-pointer text-gray-700">Requer Treinamento para o
                        uso</label>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <label for="is_active" class="cursor-pointer font-semibold text-green-700">Equipamento com Cadastro
                        Ativo</label>
                </div>
            </div>
            ...
            <div class="flex gap-4">
                <button type="submit"
                        class="bg-green-600 hover:bg-green-700 text-white px-10 py-3 rounded shadow-lg transition font-bold text-lg">
                    Salvar Tecnologia
                </button>
