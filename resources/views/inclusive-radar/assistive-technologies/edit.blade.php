...
<div class="max-w-4xl mx-auto bg-white p-6 rounded shadow">
    <div class="flex justify-between items-center mb-6 border-b pb-4">
        ...
        <span
            class="text-sm text-gray-500 font-mono bg-gray-100 px-2 py-1 rounded">ID: #{{ $assistiveTechnology->id }}</span>
    </div>
    ...
    <form action="{{ route('inclusive-radar.assistive-technologies.update', $assistiveTechnology->id) }}" method="POST"
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border p-2 rounded">{{ old('description', $assistiveTechnology->description) }}</textarea>
            </div>
            ...
            @if($assistiveTechnology->images->count() > 0)
                <div class="mt-2">
                    <label class="block font-bold mb-2 text-gray-700">Imagens Atuais
                        ({{ $assistiveTechnology->images->count() }})</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="type" value="{{ old('type', $assistiveTechnology->type) }}"
                           class="w-full border p-2 rounded">
                </div>
                <div>
                    <label class="block font-medium text-gray-700">Quantidade</label>
                    <input type="number" name="quantity" value="{{ old('quantity', $assistiveTechnology->quantity) }}"
                           class="w-full border p-2 rounded">
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="assistive_technology_status_id" class="w-full border p-2 rounded">
                        ...
                        @foreach(\App\Models\InclusiveRadar\AssistiveTechnologyStatus::where('is_active', true)->get() as $status)
            ...
            <div>
                ...
                <textarea name="notes" rows="2"
                          class="w-full border p-2 rounded">{{ old('notes', $assistiveTechnology->notes) }}</textarea>
            </div>
