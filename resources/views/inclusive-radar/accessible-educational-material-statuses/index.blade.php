...
<div class="max-w-4xl mx-auto bg-white p-6 rounded shadow">
    ...
    <a href="{{ route('inclusive-radar.accessible-educational-material-statuses.create') }}"
        ...
        @forelse($statuses as $status)
            <tr class="hover:bg-gray-50">
                ...
                <td class="border p-2 text-center space-x-2">
                    <a href="{{ route('inclusive-radar.accessible-educational-material-statuses.edit', $status) }}"
                       class="bg-yellow-500 text-white px-3 py-1 rounded">
                        Editar
                    </a>

                    @if($status->is_active)
                        <form action="{{ route('inclusive-radar.accessible-educational-material-statuses.deactivate', $status) }}"
                              method="POST"
                              class="inline">
                            @csrf
                            @method('PATCH')
                    ...
                    <form action="{{ route('inclusive-radar.accessible-educational-material-statuses.destroy', $status) }}"
                          method="POST"
                          class="inline"
